...
// ── Exam types ────────────────────────────────────────────────────────────────
const EXAM_TYPES = ["Mid-1", "Mid-2", "End-Sem", "Supplementary", "Lab Exam", "Internal"];
...
const useApiOptions = (sel) => {
  ...
  useEffect(() => {
    collegesApi.list().then(list =>
      setCollegeOpts((list || []).map(c => ({ v: c._id, l: c.name })))
    ).catch(() => {});
  }, []);

  useEffect(() => {
    if (!sel.college) { setRegulationOpts([]); setBranchOpts([]); return; }
    regulationsApi.list({ college: sel.college }).then(list =>
      setRegulationOpts((list || []).map(r => ({ v: r._id, l: r.name || r.code })))
    ).catch(() => {});
    departmentsApi.list({ college: sel.college }).then(list =>
      setBranchOpts((list || []).map(d => ({ v: d._id, l: d.code ? `${d.code} — ${d.name}` : d.name })))
    ).catch(() => {});
  }, [sel.college]);

  useEffect(() => {
    if (!sel.regulation) { setSemesterOpts([]); return; }
    semestersApi.list({ regulation: sel.regulation }).then(list =>
      setSemesterOpts((list || []).map(s => ({ v: s._id, l: `Semester ${s.number ?? s.name}` })))
    ).catch(() => {});
  }, [sel.regulation]);

  useEffect(() => {
    if (!sel.branch || !sel.semester) { setSubjectOpts([]); return; }
    subjectsApi.list({ department: sel.branch, semester: sel.semester }).then(list =>
      setSubjectOpts((list || []).map(s => ({
        v: s._id, l: s.courseCode ? `${s.courseCode} — ${s.name}` : s.name,
      })))
    ).catch(() => {});
  ...
  useEffect(() => {
    ...
    sectionsApi.list({ department: sel.branch }).then(list => {
      const mapped = (list || []).map(s => ({ v: s._id, l: `Section ${s.name}` }));
      setSectionOpts(mapped.length > 0 ? mapped : ["A","B","C","D"].map(s => ({ v: s, l: `Section ${s}` })));
    }).catch(() => setSectionOpts(["A","B","C","D"].map(s => ({ v: s, l: `Section ${s}` }))));
  }, [sel.branch]);

  useEffect(() => {
    if (!sel.subject) { setFacultyOpts([]); return; }
    facultyApi.list({ subject: sel.subject, department: sel.branch }).then(list =>
  ...
  return (
    <div>
      ...
      <div style={{ position: "relative" }}>
        ...
        >
          <option value="">
            {ready ? `Select ${label}` : `← select ${label.split(" ")[0].toLowerCase()} above`}
          </option>
          {options.map(o => (
        ...
        <span style={{
          ...
          color: ready ? C.navy : C.textMuted, fontSize: "10px",
        }}>▼</span>
      </div>
      {hint && (
        <p style={{ fontSize: "10px", color: C.warning, marginTop: "3px" }}>{hint}</p>
      )}
    </div>
  );
};
...
const ContextSummary = ({ sel, examType }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{
        fontSize: "10px", fontWeight: 800, color: C.navy,
        ...
        {chips.map(ch =>
          ch.value ? (
            <div key={ch.label} style={{
              ...
              <span style={{ fontSize: "9px", color: C.textMuted, fontWeight: 700, textTransform: "uppercase" }}>
                {ch.label}:
              </span>
              <span style={{ fontSize: "11px", fontWeight: 700, color: C.navy }}>{ch.value}</span>
            </div>
          ) : (
            <div key={ch.label} style={{
              display: "flex", alignItems: "center", gap: "4px",
              background: C.bg, border: `1px dashed ${C.border}`,
              borderRadius: "4px", padding: "4px 8px", opacity: 0.6,
            }}>
              <span style={{ fontSize: "9px", color: C.textMuted, fontWeight: 700, textTransform: "uppercase" }}>
                {ch.label}:
              </span>
...
const UploadProgress = ({ pct }) => (
  <div style={{
    ...
    <div style={{
      height: "6px", background: C.borderLight,
    ...
    {pct >= 100 && (
      <p style={{ fontSize: "11px", color: C.success, marginTop: "8px", fontWeight: 600 }}>
...
const BookletDetailModal = ({ booklet: u, onClose }) => {
  const row = (label, value) => (
    <div style={{ display: "flex", gap: "12px", padding: "7px 0", borderBottom: `1px solid ${C.borderLight}` }}>
      <span style={{ minWidth: "130px", fontSize: "11px", fontWeight: 700, color: C.textMuted, textTransform: "uppercase", letterSpacing: "0.05em" }}>
        {label}
      </span>
  ...
  return (
    ...
    >
      <div
        style={{ background: "#fff", borderRadius: 10, padding: 28, width: 480, maxWidth: "92vw", boxShadow: "0 20px 60px rgba(0,0,50,0.3)" }}
        onClick={e => e.stopPropagation()}
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div style={{ fontWeight: 800, fontSize: 16, color: C.navy }}>Booklet Details</div>
        ...
        {row("Status",         u.status)}
        {row("Uploaded",       u.uploadDate ? new Date(u.uploadDate).toLocaleString() : null)}
        {row("Evaluated By",   u.evaluatedBy?.name)}
        {row("AI Score",       u.aiResult?.totalMarks != null ? `${u.aiResult.totalMarks} / ${u.aiResult.maxMarks}` : null)}
        {row("Faculty Score",  u.facultyMarks != null ? String(u.facultyMarks) : null)}
      </div>
    </div>
  );
};
...
const UploadPage = ({ doUpload, uploadPct }) => {
  ...
  return (
    <div className="page-anim" style={{ maxWidth: "780px" }}>
      {viewBooklet && <BookletDetailModal booklet={viewBooklet} onClose={() => setViewBooklet(null)} />}
      ...
      >
        ...
        <div style={{
          ...
          <div style={{
            fontSize: "10px", fontWeight: 800, color: C.goldDark,
            textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "12px",
            display: "flex", alignItems: "center", gap: "6px",
          }}>
            <div style={{
        ...
        {/* ── Context summary ── */}
        {sel.college && (
          <ContextSummary sel={sel} examType={examType} />
        )}
      ...
      <Card title="Recent Uploads">
        {recentUploads.length === 0 ? (
          <div style={{ fontSize:"13px", color:C.textMuted, padding:"12px 0" }}>No booklets uploaded yet.</div>
          ...
          <div
            ...
            style={{
              ...
              padding: "10px 0",
              borderBottom: i < recentUploads.length - 1 ? `1px solid ${C.borderLight}` : "none",
            }}
          >
            <div style={{
              background: C.blueLight, padding: "8px",
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: "11px", color: C.textMuted, marginTop: "2px" }}>
                {u.subject?.courseCode && <Badge text={u.subject.courseCode} type="navy" />}
                &nbsp;
                {u.student?.rollNumber && <Badge text={u.student.rollNumber} type="info" />}
                &nbsp;•&nbsp;
                {u.uploadDate ? new Date(u.uploadDate).toLocaleDateString() : "—"}
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              <Badge
                ...
                type={
                  u.status === "permanently_frozen" ? "success" :
                  u.status === "frozen"             ? "success" :
                  u.status === "ai_evaluated"       ? "warning" : "info"
                }
              />
              <OutlineBtn
