...
// ── Question columns — default starting template for schema builder ────────────
//    Coordinator can edit/add/remove columns in Step 3.
const INIT_COLS = [
...
// ─────────────────────────────────────────────────────────────────────────────
function Step1UploadQP({ onNext, qpFile, setQpFile, selectedEvent }) {
  const [stage, setStage] = useState(qpFile ? "done" : "idle");
  const [progress, setProgress] = useState(0);
  const fileRef = useRef();

  const subjectLabel = selectedEvent
    ? `${selectedEvent.subject?.name || "Subject"} (${selectedEvent.subject?.code || ""})`
    : "Question Paper";
  ...
  const useActualFiles = () =>
    runExtraction({ name:`qp_1.jpeg + qp_2.jpeg  (${subjectLabel})` });
            ...
            <div style={{ background:"#eaf3fb", border:`1px solid #bdd9f0`, borderRadius:10,
              ...
              <div>
                <div style={{ fontWeight:700, fontSize:13.5, color:C.blue }}>
                  📎 Detected: {selectedEvent.subject?.name} QP files
                </div>
                <div style={{ fontSize:12, color:C.sub, marginTop:3 }}>
                  {selectedEvent.subject?.name} ({selectedEvent.subject?.code}) ·{" "}
                  {selectedEvent.type?.toUpperCase()} ·{" "}
                  {selectedEvent.examDate ? new Date(selectedEvent.examDate).toLocaleDateString() : "—"}
                </div>
              </div>
          ...
          <div style={{ background:"#e6f7ef", border:`1px solid #86efac`, borderRadius:10,
            ...
            <div>
              <div style={{ fontWeight:700, fontSize:13.5, color:C.green }}>
                ✅ Extracted — {selectedEvent?.subject?.name?.toUpperCase() || "QUESTION PAPER"}{selectedEvent?.subject?.code ? ` (${selectedEvent.subject.code})` : ""}
              </div>
            ...
            <tbody>
              {studentRows.map((row,ri)=>(
                <tr key={ri} style={{ background:ri%2===0?"#fff":"#fafbff" }}>
                  <td style={td({ fontWeight:600, color:C.sub })}>{ri+1}</td>
                  <td style={td({ fontWeight:600, color:C.text, fontSize:12 })}>{row.rollNumber}</td>
                  <td style={td({ color:C.sub, fontSize:12 })}>{row.omrCode}</td>
              ...
              {studentRows.length === 0 && (
                <tr>
                  <td colSpan={3 + cols.length + 1} style={td({ color:C.muted, fontStyle:"italic", fontSize:12 })}>
                    No students loaded — select an exam event above.
                  </td>
                </tr>
              )}
              {[...Array(3)].map((_,i)=>(
                <tr key={`g${i}`} style={{ opacity:0.28 }}>
                  <td style={td({ color:C.muted })}>{studentRows.length+i+1}</td>
...
// ─────────────────────────────────────────────────────────────────────────────
function Step4Publish({ onBack, cols, qpFile, selectedEvent, studentRows }) {
  ...
  const publish = async () => {
    ...
    setPublishing(true);
    try {
      // Upload question paper file if present
      if (qpFile && qpFile instanceof File) {
        const fd = new FormData();
        fd.append("file", qpFile);
        fd.append("examEvent", selectedEvent._id);
        await questionPapers.upload(fd).catch(() => {});
      }

      // Save evaluation schema
      await evaluationSchemas.create({
        examEvent: selectedEvent._id,
  ...
  const subjectName = selectedEvent?.subject?.name || "—";
  const subjectCode = selectedEvent?.subject?.code || "—";
  const examType    = selectedEvent?.type ? selectedEvent.type.toUpperCase() : "—";
  const examDate    = selectedEvent?.examDate
    ? new Date(selectedEvent.examDate).toLocaleDateString()
    : "—";
  ...
  return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))", gap:10, marginBottom:"1.25rem" }}>
        {[
          { l:"Subject",       v:subjectName   },
          { l:"Code",          v:subjectCode   },
          { l:"Exam",          v:examType      },
          { l:"Date",          v:examDate      },
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        ...
        <div style={{ display:"flex", gap:10, alignItems:"center" }}>
          ...
          <Btn variant={published?"ghost":"gold"} size="lg" onClick={publish}
            disabled={published || publishing || !selectedEvent}>
            {publishing ? "Publishing…" : published ? "✅ Schema Published!" : "🚀 Publish Schema"}
          </Btn>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
export default function SCDashboard({ user }) {
  ...
  // API state
  const [examEventList,    setExamEventList]   = useState([]);
  ...
  // Derived
  const selectedEvent = examEventList.find(e => e._id === selectedEventId) || null;
  const schemaPublished = step >= 3;

  // ── Fetch exam events on mount ──────────────────────────────────────────────
  useEffect(() => {
    if (!userId) { setEventsLoading(false); return; }
    setEventsLoading(true);
    examEvents.list({ assignedTo: userId, status: "active" })
      .then(list => setExamEventList(Array.isArray(list) ? list : []))
      .catch(() => setExamEventList([]))
      .finally(() => setEventsLoading(false));
  }, [userId]);

  // ── Fetch students when an event is selected ────────────────────────────────
  useEffect(() => {
    if (!selectedEventId) { setStudentRows([]); return; }
    setStudentsLoading(true);
    students.list({ examEvent: selectedEventId, limit: 100 })
      .then(list => setStudentRows(Array.isArray(list) ? list : []))
      .catch(() => setStudentRows([]))
      .finally(() => setStudentsLoading(false));
  }, [selectedEventId]);

  // ── Fetch upload history when History tab opens ─────────────────────────────
  useEffect(() => {
    if (activeTab !== "history" || !userId) return;
    setHistoryLoading(true);
    questionPapers.list({ uploadedBy: userId })
  ...
  return (
    <div className="page-anim" style={{ display:"flex", flexDirection:"column", gap:0 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:14, marginBottom:20 }}>
        <MiniStat value={examEventList.length} label="Subjects Coordinating" icon="📚" color={C.navy}  />
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderTop:"none",
        ...
        {activeTab==="wizard" && (
          <>
            ...
            <div style={{ marginBottom:"1.25rem" }}>
              ...
              {eventsLoading ? (
                <div style={{ fontSize:13, color:C.muted }}>Loading assigned exam events…</div>
              ) : (
                <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
                  <label style={{ fontSize:12, fontWeight:700, color:C.sub, textTransform:"uppercase", letterSpacing:"0.06em" }}>
                    Exam Event:
                  </label>
                  <select
                    ...
                    onChange={e => {
                      ...
                      // Reset wizard on event change
                      setStep(0); setQpFile(null); setAnsFile(null); setCols(INIT_COLS);
                    }}
                    style={{ padding:"7px 12px", borderRadius:7, border:`1.5px solid ${selectedEventId?C.navy:C.border}`,
                      fontSize:13, color:selectedEventId?C.text:C.muted, fontFamily:"inherit",
                      background:"#fff", cursor:"pointer" }}>
                    <option value="">— Select an exam event —</option>
                    {examEventList.map(ev=>(
                      <option key={ev._id} value={ev._id}>
                        {ev.subject?.name} ({ev.subject?.code}) · {ev.type?.toUpperCase()} ·{" "}
                        Sem {ev.semester?.number} · {ev.regulation?.name} ·{" "}
                        {ev.examDate ? new Date(ev.examDate).toLocaleDateString() : "TBD"}
                      </option>
                    ))}
                  </select>
                  {studentsLoading && (
                    <span style={{ fontSize:12, color:C.muted }}>Loading students…</span>
                  )}
                </div>
              )}
              ...
              {selectedEvent && (
                <div style={{ fontSize:13, color:C.sub, marginTop:6 }}>
                  {selectedEvent.subject?.name} ({selectedEvent.subject?.code}) ·{" "}
                  {selectedEvent.type?.toUpperCase()} · Sem {selectedEvent.semester?.number} ·{" "}
                  {selectedEvent.regulation?.name} ·{" "}
                  {selectedEvent.examDate ? new Date(selectedEvent.examDate).toLocaleDateString() : "TBD"}
                </div>
              )}
            </div>
            ...
            {!selectedEventId && !eventsLoading ? (
              <InfoBox type="warning">
                Please select an exam event above to start the wizard.
              </InfoBox>
            ) : selectedEventId && (
              <>
                <StepBar step={step} />
        ...
        {activeTab==="subjects" && (
          <>
            ...
            ) : examEventList.length === 0 ? (
              <InfoBox type="info">No active exam events assigned to you.</InfoBox>
            ) : (
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))", gap:14 }}>
                {examEventList.map((ev, i) => {
                  const isSelected  = ev._id === selectedEventId;
                  const qpOk        = isSelected && !!qpFile;
                  const schemaOk    = isSelected && schemaPublished;
                  return (
                    <div key={ev._id} style={{ background:C.bg, border:`1px solid ${C.border}`, borderRadius:12,
                      padding:"16px 18px", cursor:"pointer", transition:"box-shadow .2s",
                      borderTop:`3px solid ${schemaOk?C.green:qpOk?C.blue:C.gold}` }}
                      ...
                      <div style={{ fontWeight:800, fontSize:15, color:C.text, marginBottom:4 }}>
                        {ev.subject?.name || "Subject"}
                      </div>
                      <div style={{ fontSize:12, color:C.sub, marginBottom:10 }}>
                        {ev.subject?.code} · Sem {ev.semester?.number} ·{" "}
                        {ev.type?.toUpperCase()}
                      </div>
                      <div style={{ display:"flex", gap:8, marginBottom:12, flexWrap:"wrap" }}>
                        <Pill color={qpOk?C.green:C.muted}>QP: {qpOk?"Uploaded":"Pending"}</Pill>
                        <Pill color={schemaOk?C.green:C.muted}>Schema: {schemaOk?"Published":"Pending"}</Pill>
                      </div>
        ...
        {activeTab==="history" && (
          <>
            ...
            {historyLoading ? (
              ...
              <InfoBox type="info">No uploads found.</InfoBox>
            ) : (
              <div style={{ overflowX:"auto" }}>
                  ...
                  <tbody>
                    {uploadHistory.map((r,i)=>(
                      <tr key={i} style={{ borderBottom:`1px solid #f0f2f8` }}>
                        <td style={{ padding:"10px 14px", fontWeight:600, color:C.blue }}>
                          📎 {r.filename || r.file?.name || r.originalName || "—"}
                        </td>
                        <td style={{ padding:"10px 14px" }}>
                          {r.examEvent?.subject?.name || r.subject || "—"}
                        </td>
                        <td style={{ padding:"10px 14px" }}>
                          <Pill color={C.purple}>{r.type || "Question Paper"}</Pill>
                        </td>
                        <td style={{ padding:"10px 14px", color:C.sub }}>
                          {r.createdAt ? new Date(r.createdAt).toLocaleDateString() : "—"}
                        </td>
                        <td style={{ padding:"10px 14px" }}>
                          <Pill color={r.status==="done"||r.status==="processed"?C.green:C.muted}>
                            {r.status || "Uploaded"}
                          </Pill>
                        </td>
                      </tr>
