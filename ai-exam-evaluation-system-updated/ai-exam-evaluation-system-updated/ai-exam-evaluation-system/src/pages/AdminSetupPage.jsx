...
const StudentsTab = ({ toast, colleges }) => {
  ...
  const addStudent = async () => {
    ...
    try {
      await auth.register({
        ...
        currentSemester: parseInt(form.semester) || 1,
      });
      ...
      if (profile) {
        await sectionsApi.addStudent(sectionId, profile._id).catch(() => {});
        setStudents(prev => [...prev, profile]);
      } else {
        setStudents(prev => [...prev, {
          name: form.name, email: form.email,
          rollNumber: form.rollNumber, currentSemester: parseInt(form.semester) || 1,
        }]);
      }
      setForm({ name: "", email: "", rollNumber: "", semester: "", password: "" });
      toast.show(`Student "${form.name}" added.`);
    } catch (e) {
  ...
  const selectedSectionName = sections.find(s => s._id === sectionId)?.name || "";
    ...
    <>
      <Card title="Select College → Department → Section" style={{ marginBottom: 16 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, marginBottom: deptId ? 14 : 0 }}>
          ...
          <Sel label="Section"    value={sectionId} onChange={setSectionId} list={sections.map(s => ({ _id: s._id, name: `Section ${s.name}` }))} placeholder={deptId ? "Select section" : "—"} />
        </div>
        {deptId && (
          <div style={{ display: "flex", gap: 10, alignItems: "center", borderTop: `1px solid ${C.borderLight}`, paddingTop: 12 }}>
            <input value={newSection} onChange={e => setNewSection(e.target.value.toUpperCase())} placeholder="New section (e.g. A, B)" style={{ ...inp, flex: 1 }} onKeyDown={e => e.key === "Enter" && addSection()} />
      ...
      {sectionId && (
        <Card title="Add Student" style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
            ...
            <input value={form.semester}   onChange={e => setForm(f => ({ ...f, semester: e.target.value }))}   placeholder="Semester (1–8)"           style={inp} type="number" min={1} max={8} />
            <input value={form.password}   onChange={e => setForm(f => ({ ...f, password: e.target.value }))}   placeholder="Password (default: Welcome@123)" style={inp} type="password" />
          </div>
      ...
      {sectionId && (
        <Card title={loading ? "Students — Loading…" : `Students — Section ${selectedSectionName} (${students.length})`}>
          {loading ? (
            <div style={{ padding: 16, color: C.textMuted, fontSize: 13 }}>Loading…</div>
          ) : students.length === 0 ? (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ background: C.bg }}>
                  {["#", "Name", "Roll No", "Email", "Semester"].map(h => (
                    <th key={h} style={{ padding: "8px 10px", textAlign: "left", fontSize: 11, fontWeight: 800, color: C.textMuted, textTransform: "uppercase", letterSpacing: "0.05em" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {students.map((s, i) => (
                  <tr key={s._id} style={{ borderTop: `1px solid ${C.borderLight}` }}>
                    ...
                    <td style={{ padding: "9px 10px" }}><Badge text={`Sem ${s.currentSemester || "—"}`} type="info" /></td>
                  </tr>
                ))}
              </tbody>
            </table>
